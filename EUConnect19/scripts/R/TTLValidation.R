...
# DESC: LDWorkshop: Select, query, validate, and visualize a TTL file.
# IN  : 
# OUT : N/A
# REQ : 
# NOTE: Visualize requires query to return s,p,o. 
#       Valid NCT values are hard coded. Source: ClassInfo.xls. TODO: read direct
#         from the XLS instead of hard code there.
# TODO: 
###############################################################################
library(DT)
library(plyr)     #  rename
...
server <- function(input, output, session) {
    ...
    queryText <- observeEvent(input$fileRQ, {
      ...
      return(queryText) # return the text to displayin text Outputs
    })
    output$query <- renderPrint({ queryText() })    

    data <- eventReactive(input$runQuery, {
      # Reset QC Result dataframe
      qcObs <<- data.frame(Examine = character(), 
                           Value   = character(), 
                           Message = character())
      inFileTTL <- input$fileTTL
      rdf <- rdf_parse(inFileTTL$datapath, format = 'turtle')
      triples <- rdf_query(rdf, input$query)
    })
